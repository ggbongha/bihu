  ...
  <div class="container">
    ...
    <section>
      <article v-for="article in articles">
        <h3>
          <router-link :to="{ name:'ArticleShow', params: { id: article.id }}">标题：{{ article.get('title') }}</router-link>
          <router-link :to="{name: 'User', params: { id: article.get('author').id}}">作者：{{ article.get('author').get('username') }}</router-link>
          <router-link :to="{name: 'ArticleShow', params: { id: article.createdAt}}">时间：{{ article.get('createdAt')}}</router-link>
        </h3>
      </article>
    </section>
  </div>
